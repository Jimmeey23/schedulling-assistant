...
DATA_DRIVEN_DAILY_RANGES: Dict[str, Dict[str, tuple]] = {
    "Kwality House, Kemps Corner": {
        "Monday": (11, 13), "Tuesday": (9, 11), "Wednesday": (9, 13),
        "Thursday": (10, 12), "Friday": (7, 11), "Saturday": (9, 11), "Sunday": (4, 8),
    },
    "Supreme HQ, Bandra": {
        "Monday": (9, 12), "Tuesday": (10, 11), "Wednesday": (9, 10),
        "Thursday": (9, 12), "Friday": (10, 11), "Saturday": (9, 12), "Sunday": (6, 7),
    },
    "Kenkere House": {
        "Monday": (6, 7), "Tuesday": (6, 7), "Wednesday": (7, 8),
        "Thursday": (7, 7), "Friday": (5, 7), "Saturday": (7, 8), "Sunday": (4, 4),
    },
}
...
MAX_FORMAT_PER_DAY: Dict[str, int] = {
    ...
    "Studio Strength Lab": 1,
    "Studio Strength Lab (Push)": 1,
    "Studio Strength Lab (Pull)": 1,
    "Studio Strength Lab (Full Body)": 1,
...
WEEKLY_FORMAT_MINIMUMS: Dict[str, int] = {
    ...
    "Studio Strength Lab": 4,      # Increased for Kwality - specialized classes
...
class ScheduleOptimiser:
    ...
    def _available_days(self, trainer, location, default):
        for ov in self.overrides.get("available_days_overrides", []):
            if ov["trainer"] == trainer and ov["location"] == location:
                return ov["days"]
        return default
    ...
    def _max_per_day(self, trainer, location, default):
        for ov in self.overrides.get("max_classes_overrides", []):
            if ov["trainer"] == trainer and (not ov.get("location") or ov["location"] == location):
    ...
    def run(self) -> dict:
        ...
        raw_profiles = {p["name"]: p for p in profiles_list}
    ...
    def _pick_daily_target(self, location: str, day_name: str) -> int:
        ...
        ai_overrides = self.ai_brief.get("daily_target_overrides", {})
    ...
    def _schedule_day(self, location, day_name, date_str, target_count,
        ...
        for p in self._get_pinned_slots(location, day_name):
            ...
            sl_today = sum(1 for s in slots_today if "Strength Lab" in s.class_name)
            if "Strength Lab" in cname and wcc_for_caps.get("Studio Strength Lab", 0) + sl_today >= 4:
                continue
            hh_today = sum(1 for s in slots_today if "HIIT" in s.class_name)
            if "HIIT" in cname and wcc_for_caps.get("Studio HIIT", 0) + hh_today >= 3:
                continue
            amp_today = sum(1 for s in slots_today if "Amped Up" in s.class_name)
            if "Amped Up" in cname and wcc_for_caps.get("Studio Amped Up!", 0) + amp_today >= 2:
                continue
            # Back Body Blaze (all variants) weekly cap for Kwality
            if location == "Kwality House, Kemps Corner" and "Back Body" in cname:
                bb_today = sum(1 for s in slots_today if "Back Body" in s.class_name)
                bb_week = sum(v for k, v in wcc_for_caps.items() if "Back Body" in k) + bb_today
                if bb_week >= 4:
                    continue
            ...
            slot = ScheduleSlot(
                ...
                predicted_fill_rate=hist.get("avg_fill_rate", 0.52),
        ...
        for r in sorted(self.protected.get((location, DOW_REVERSE[day_name]), []),
            ...
            trainer = r["trainer"]
            cname = r["class"]
            # PROTECT_EXACT bypasses per-day format cap (top 3% performer must be placed).
            # But hard weekly caps still apply: Strength Lab ≤4/wk, HIIT ≤3/wk, Amped Up ≤2/wk.
            wcc = weekly_class_counts or {}
            sl_today = sum(1 for s in slots_today if "Strength Lab" in s.class_name)
            sl_week = wcc.get("Studio Strength Lab", 0) + sl_today
            if "Strength Lab" in cname and sl_week >= 2:
                continue
            hiit_today = sum(1 for s in slots_today if "HIIT" in s.class_name)
            hiit_week = wcc.get("Studio HIIT", 0) + hiit_today
            if "HIIT" in cname and hiit_week >= 3:
                continue
            amped_today = sum(1 for s in slots_today if "Amped Up" in s.class_name)
            amped_week = wcc.get("Studio Amped Up!", 0) + amped_today
            if "Amped Up" in cname and amped_week >= 2:
                continue
            # Back Body Blaze (all variants) weekly cap for Kwality
            if location == "Kwality House, Kemps Corner" and "Back Body" in cname:
                bb_today = sum(1 for s in slots_today if "Back Body" in s.class_name)
                bb_week = sum(v for k, v in wcc.items() if "Back Body" in k) + bb_today
                if bb_week >= 4:
                    continue
            if self._on_leave(trainer, date_str, location):
                continue
            ...
            slot = ScheduleSlot(
                ...
                class_name=cname, trainer_1=trainer, trainer_2="", cover="",
                room=room, capacity=rooms[room]["capacity"], duration_min=dur,
                predicted_fill_rate=hist.get("avg_fill_rate", 0.50),
        ...
        # ---- Phase 2b: PowerCycle enforcement for Supreme (SU-001: min 2 PC/weekday; floor 14/wk) ----
    ...
    def _fill_slot(self, location, day_name, date_str, time_str, used_at_time,
        ...
        def _diversity_adjustment(cname: str) -> float:
            ...
            wmin = WEEKLY_FORMAT_MINIMUMS.get(cname, 0)
            weekly_so_far = wcc.get(cname, 0)
        ...
        def _build_candidates(day_filter: bool, allow_drop: bool, seen_tc: Optional[set] = None):
            ...
            for r in self.scores_data["class_slot_ranking"]:
                ...
                if "PowerCycle" in cname and location == "Kwality House, Kemps Corner":
                    if pc_kwality_week >= 10:  # raised cap — dedicated room, underused
                        continue
                # Back Body Blaze (all variants) weekly cap for Kwality
                if location == "Kwality House, Kemps Corner" and "Back Body" in cname:
                    bb_today = sum(1 for s in slots_today if "Back Body" in s.class_name)
                ...
                if "HIIT" in cname:
                    hiit_so_far = wcc.get("Studio HIIT", 0) + sum(1 for s in slots_today if "HIIT" in s.class_name)
                    if hiit_so_far >= 3:
                        continue
                if "Amped Up" in cname:
                    amped_so_far = wcc.get("Studio Amped Up!", 0) + sum(1 for s in slots_today if "Amped Up" in s.class_name)
                    if amped_so_far >= 2:
                        continue
                ...
                effective_score = (r["score"] + shift_bonus + div_adj + hours_bonus
                                   + popularity_bonus + ai_delta
                                   - time_penalty + self._score_noise(rec_for_noise))
        ...
        return ScheduleSlot(
            ...
            duration_min=dur,
            predicted_fill_rate=hist.get("avg_fill_rate", 0.38),
            score=best_r["score"], recommendation=rec,
    ...
    def _fallback_slot(self, location, day_name, date_str, time_str,
        ...
        for cname, fam, dur in fallback_candidates:
            ...
            if "Strength Lab" in cname:
                sl_today = sum(1 for s in st_today if "Strength Lab" in s.class_name)
                if wcc.get("Studio Strength Lab", 0) + sl_today >= 4:
                    continue
            if "HIIT" in cname:
                hh_today = sum(1 for s in st_today if "HIIT" in s.class_name)
                if wcc.get("Studio HIIT", 0) + hh_today >= 3:
                    continue
            if "Amped Up" in cname:
                amp_today = sum(1 for s in st_today if "Amped Up" in s.class_name)
                if wcc.get("Studio Amped Up!", 0) + amp_today >= 2:
                    continue
            # Back Body Blaze (all variants) weekly cap for Kwality
            if location == "Kwality House, Kemps Corner" and "Back Body" in cname:
                bb_today = sum(1 for s in st_today if "Back Body" in s.class_name)
                bb_week = sum(v for k, v in wcc.items() if "Back Body" in k) + bb_today
                if bb_week >= 4:
                    continue
            if location == "Kenkere House" and "PowerCycle" in cname:
            ...
            for name, state in sorted(self.trainer_states.items(), key=lambda x: priority(x[0], x[1])):
                ...
                return ScheduleSlot(
                    ...
                    room=room, capacity=rooms.get(room, {}).get("capacity", 15),
                    duration_min=dur,
                    predicted_fill_rate=0.35,
                    score=35.0, recommendation="INCLUDE", is_experimental=False,
    ...
    def _fill_slot_class(self, location, day_name, date_str, time_str, class_name, class_fam,
        ...
        if "Strength Lab" in class_name:
            ...
            if wcc.get("Studio Strength Lab", 0) + sl_today >= 4:
                return None
        if "HIIT" in class_name:
            hh_today = sum(1 for s in slots_today if "HIIT" in s.class_name)
            if wcc.get("Studio HIIT", 0) + hh_today >= 3:
                return None
        if "Amped Up" in class_name:
            amp_today = sum(1 for s in slots_today if "Amped Up" in s.class_name)
            if wcc.get("Studio Amped Up!", 0) + amp_today >= 2:
                return None
        # Back Body Blaze (all variants) weekly cap for Kwality
        if location == "Kwality House, Kemps Corner" and "Back Body" in class_name:
            bb_today = sum(1 for s in slots_today if "Back Body" in s.class_name)
            bb_week = sum(v for k, v in wcc.items() if "Back Body" in k) + bb_today
            if bb_week >= 4:
                return None
        ...
        return ScheduleSlot(
            ...
            duration_min=dur,
            predicted_fill_rate=hist.get("avg_fill_rate", 0.45),
            score=best_r["score"], recommendation=best_r.get("recommendation", "INCLUDE"),
