    ...
    for edit in edits:
        ...
        try:
            ...
            if action == "remove":
                ...
                if not found:
                    results.append({"action": action, "error": f"Slot not found: {edit.get('day')} {edit.get('time')} {edit.get('class_name')}"})
                    continue
                rows.remove(found)
                dirty = True
                ...
                found = _fuzzy_find_slot(rows, edit)
                if not found:
                    results.append({"action": action, "error": f"Slot not found: {edit.get('day')} {edit.get('time')} {edit.get('class_name')} {edit.get('trainer_1')}"})
                    continue
                ...
                found["scheduling_reason"] = edit.get("reason") or f"NL edit: {old_trainer} → {found.get('trainer_1','?')}"
                dirty = True
                results.append({"action": action, "ok": True})
...
def _find_schedule_row(rows, slot):
    if not isinstance(rows, list):
        return -1
    required = ("location", "day_of_week", "time", "class_name", "trainer_1")
    for idx, row in enumerate(rows):
        if all((not slot.get(key)) or (row.get(key) or "") == (slot.get(key) or "") for key in required):
            if slot.get("room") and (row.get("room") or "") != slot.get("room"):
                continue
            if slot.get("date") and (row.get("date") or "") != slot.get("date"):
                continue
            return idx
    return -1


def _operation_result(operation, status, message, before=None, after=None):
    return {
...
def _apply_ai_schedule_operations(data: dict, iteration: str, operations: list[dict]) -> dict:
    ...
    for operation in operations or []:
        ...
        slot_ref = operation.get("slot") or {}
        loc = slot_ref.get("location") or operation.get("location")
        if op_type == "add_class":
            candidate = dict(operation.get("slot") or {})
            loc = candidate.get("location")
            if not loc:
                rejected.append(_operation_result(operation, "rejected", "Missing location for add_class"))
                continue
            try:
                candidate.setdefault("recommendation", "AI_OPTIMIZED")
                candidate.setdefault("manual_added", True)
                candidate["ai_optimized"] = True
                candidate["scheduling_reason"] = operation.get("reason") or "AI optimizer added this class"
                _validate_manual_slot(data, iteration, candidate)
                _rows_for_iteration(data, iteration, loc).append(candidate)
                applied.append(_operation_result(operation, "applied", "Class added", after=candidate))
            except Exception as exc:
                rejected.append(_operation_result(operation, "rejected", str(exc), after=candidate))
            continue

        if not loc:
            rejected.append(_operation_result(operation, "rejected", "Missing slot location"))
            continue
        rows = _rows_for_iteration(data, iteration, loc)
        idx = _find_schedule_row(rows, slot_ref)
        if idx < 0:
            rejected.append(_operation_result(operation, "rejected", "Could not find referenced class"))
            continue
        ...
        try:
            if op_type == "remove_class":
                rows.pop(idx)
                applied.append(_operation_result(operation, "applied", "Class removed", before=before))
            elif op_type == "swap_trainer":
                new_trainer = str(operation.get("new_trainer") or "").strip()
                if not new_trainer:
                    raise ValueError("Missing new_trainer")
                candidate = dict(before)
                candidate["replaced_from_trainer"] = before.get("trainer_1", "")
                candidate["trainer_1"] = new_trainer
                candidate["recommendation"] = "AI_OPTIMIZED"
                candidate["ai_optimized"] = True
                candidate["scheduling_reason"] = operation.get("reason") or f"AI optimizer trainer swap to {new_trainer}"
                _validate_manual_slot(data, iteration, candidate, original_slot=before)
                rows[idx] = candidate
                applied.append(_operation_result(operation, "applied", "Trainer swapped", before=before, after=candidate))
                ...
                moved.update({
                    ...
                    "day_of_week": target["day_of_week"],
                    "time": target["time"],
                    "room": target.get("room", moved.get("room", "")),
                    "recommendation": "AI_OPTIMIZED",
                    "manual_moved": True,
                    "ai_optimized": True,
                    "scheduling_reason": operation.get("reason") or "AI optimizer moved this class",
                })
                rows.pop(idx)
                try:
                    _validate_manual_slot(data, iteration, moved, original_slot=before)
                    _rows_for_iteration(data, iteration, target["location"]).append(moved)
                except Exception:
                    _rows_for_iteration(data, iteration, loc).insert(idx, before)
                    raise
                applied.append(_operation_result(operation, "applied", "Class moved", before=before, after=moved))
            elif op_type == "change_class":
                new_class = str(operation.get("new_class") or operation.get("class_name") or "").strip()
                if not new_class:
                    raise ValueError("Missing new_class")
                candidate = dict(before)
                candidate["changed_from_class"] = before.get("class_name", "")
                candidate["class_name"] = new_class
                candidate["duration_min"] = int(operation.get("duration_min") or candidate.get("duration_min") or 57)
                candidate["recommendation"] = "AI_OPTIMIZED"
                candidate["ai_optimized"] = True
                candidate["scheduling_reason"] = operation.get("reason") or f"AI optimizer changed class to {new_class}"
                _validate_manual_slot(data, iteration, candidate, original_slot=before)
                rows[idx] = candidate
                applied.append(_operation_result(operation, "applied", "Class changed", before=before, after=candidate))
            else:
                rejected.append(_operation_result(operation, "rejected", f"Unsupported operation type: {op_type}", before=before))
        except Exception as exc:
            rejected.append(_operation_result(operation, "rejected", str(exc), before=before))

    return {"applied": applied, "rejected": rejected}


def _extract_optimizer_json(raw: str) -> dict:
    ...
    if start < 0 or end < start:
        raise ValueError("AI optimizer did not return a JSON object")
    return json.loads(text[start:end + 1])


def _latest_schedule_payload() -> dict:
    path = WEB_DIR / "schedule_data.json"
    if not path.exists():
        return {}
...
def _build_schedule_optimizer_prompt(payload: dict) -> str:
    schedule_data = _latest_schedule_payload()
    iteration = payload.get("iteration") or "Main"
    active_location = payload.get("location") or ""
    rows = []
    if iteration == "Main":
        for loc, loc_rows in (schedule_data.get("locations") or {}).items():
            if active_location and loc != active_location:
                continue
            rows.extend(loc_rows or [])
    else:
        loc_map = (schedule_data.get("iterations") or {}).get(iteration) or {}
        for loc, loc_rows in loc_map.items():
            if active_location and loc != active_location:
                continue
            rows.extend(loc_rows or [])
    compact_rows = [
        {
            ...
            "trainer_1": r.get("trainer_1"),
            "room": r.get("room"),
            "duration_min": r.get("duration_min"),
            "score": r.get("score"),
            "predicted_fill_rate": r.get("predicted_fill_rate"),
            "historical_avg_fill": r.get("historical_avg_fill"),
            "historical_session_count": r.get("historical_session_count"),
            "constraint_violations": r.get("constraint_violations", []),
        }
        for r in rows[:220]
    ]
    return (
        "You are the Physique 57 India schedule optimization engine. Analyze the current schedule and propose only high-confidence, operationally useful changes.\n"
        "Return JSON only with this schema: {\"summary\":\"...\",\"operations\":[...]}. "
        "Allowed operation types: swap_trainer, remove_class, move_class, add_class, change_class. "
        "Every operation will be server-validated against trainer availability, overlaps, location movement, daily caps, weekly hours, and room constraints before it is applied. "
        "Prefer fixing low score, low fill, repeated class mix, bad 8-9am placement, trainer overload, and obvious empty peak opportunities. "
        "Do not invent trainer names, class names, rooms, days, or times. Keep the patch small: maximum 12 operations.\n\n"
        "Operation examples:\n"
        "{\"type\":\"swap_trainer\",\"reason\":\"...\",\"slot\":{\"location\":\"...\",\"day_of_week\":\"Monday\",\"time\":\"09:00\",\"class_name\":\"Studio Barre 57\",\"trainer_1\":\"Old Trainer\"},\"new_trainer\":\"New Trainer\"}\n"
        "{\"type\":\"move_class\",\"reason\":\"...\",\"slot\":{\"location\":\"...\",\"day_of_week\":\"Monday\",\"time\":\"09:00\",\"class_name\":\"Studio Barre 57\",\"trainer_1\":\"Trainer\"},\"target\":{\"location\":\"...\",\"day_of_week\":\"Monday\",\"time\":\"08:45\",\"date\":\"2026-05-04\",\"room\":\"studio_a\"}}\n"
        "{\"type\":\"remove_class\",\"reason\":\"...\",\"slot\":{\"location\":\"...\",\"day_of_week\":\"Monday\",\"time\":\"09:00\",\"class_name\":\"Studio Barre 57\",\"trainer_1\":\"Trainer\"}}\n"
        "{\"type\":\"add_class\",\"reason\":\"...\",\"slot\":{\"location\":\"...\",\"date\":\"2026-05-04\",\"day_of_week\":\"Monday\",\"time\":\"08:30\",\"class_name\":\"Studio PowerCycle\",\"trainer_1\":\"Trainer\",\"room\":\"powercycle\",\"capacity\":14,\"duration_min\":45}}\n"
        "{\"type\":\"change_class\",\"reason\":\"...\",\"slot\":{\"location\":\"...\",\"day_of_week\":\"Monday\",\"time\":\"09:00\",\"class_name\":\"Old Class\",\"trainer_1\":\"Trainer\"},\"new_class\":\"Studio Mat 57\",\"duration_min\":57}\n\n"
        f"REQUEST CONTEXT: {json.dumps(payload, ensure_ascii=False)}\n"
        f"CURRENT SCHEDULE ROWS: {json.dumps(compact_rows, ensure_ascii=False)}"
    )
...
def _call_schedule_optimizer_ai(payload: dict) -> dict:
    ...
    if not OPENAI_AVAILABLE:
        raise RuntimeError("AI client not available. Install the openai package and configure an AI key.")
    client, settings = create_ai_client()
    if not client or not settings:
        ...
        if api_key:
            ...
            settings = {"model": runtime.get("model") or "openai/gpt-oss-120b:free"}
        else:
            raise RuntimeError("Add an AI API key in Control Center before optimizing.")
    response = client.chat.completions.create(
        model=(settings or {}).get("model") or "openai/gpt-oss-120b:free",
        messages=[
            {"role": "system", "content": "Return only strict JSON. You optimize schedules through validated patch operations."},
            {"role": "user", "content": _build_schedule_optimizer_prompt(payload)},
        ],
        temperature=0.2,
    )
    message = response.choices[0].message if response.choices else None
    return _extract_optimizer_json(message.content if message else "")
...
def nl_edit():
    try:
        ...
        if not instruction:
            return _json({"error": "Empty instruction"}, 400)
        ...
        try:
            ...
            pass
        if not client:
            cfg = _saved_ai_runtime_settings()
            ...
            if api_key:
                ...
                settings = {"model": cfg.get("model", "openai/gpt-oss-120b:free")}
        if not client:
            return _json({"error": "AI not configured. Add OPENROUTER_API_KEY or set API key in Settings."})
...
def nl_edit_apply():
    try:
        ...
        iteration = payload.get("iteration") or "Main"
        if not edits:
            return _json({"error": "No edits provided"}, 400)
...
@app.route("/api/optimise-schedule", methods=["POST"])
def optimize_schedule():
    try:
        ...
        path = WEB_DIR / "schedule_data.json"
        if not path.exists():
            return _json({"ok": False, "error": "web/schedule_data.json was not found"}, 404)
        data = json.loads(path.read_text())
        ai_plan = _call_schedule_optimizer_ai(payload)
        result = _apply_ai_schedule_operations(data, iteration, ai_plan.get("operations", []))
        supabase_saved = _write_schedule_data(data) if result["applied"] else {"saved": False}
        return _json({
            "ok": True,
